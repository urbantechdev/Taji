...
import { generateReceiptQRDataURL, generateReceiptBarcodeDataURL } from './realQrBarcode';
...
export async function exportBillingDocumentPDF(
  order: SaleOrder,
  etrConfig: ETRConfig,
  locations: LocationInfo[],
  ...
  // Generate genuine scannable QR Code and Code128 Barcode for the fiscal document
  let qrDataUrl = '';
  let barcodeDataUrl = '';
  try {
    qrDataUrl = await generateReceiptQRDataURL(
      order.receiptNumber || order.id,
      etrConfig.taxPin,
      order.grandTotal
    );
    barcodeDataUrl = generateReceiptBarcodeDataURL(
      order.receiptNumber || order.id
    );
  } catch (codeErr) {
    console.warn('Error generating QR/Barcode for PDF export:', codeErr);
  }
  ...
  if (isDeliveryNote) {
    ...
    // Delivery Note Genuine Barcode & QR Code Footer
    const dnFootY = sigY + 70;
    if (qrDataUrl) {
      try {
        doc.addImage(qrDataUrl, 'PNG', 40, dnFootY, 34, 34);
      } catch (e) {
        console.warn('Failed to add QR image to delivery note:', e);
      }
    }
    if (barcodeDataUrl) {
      try {
        doc.addImage(barcodeDataUrl, 'PNG', 85, dnFootY + 2, 140, 30);
      } catch (e) {
        console.warn('Failed to add barcode image to delivery note:', e);
      }
    }
    doc.setFontSize(7);
    doc.setTextColor(100, 116, 139);
    doc.setFont('helvetica', 'normal');
    doc.text(`Official Dispatch Handover Reference: ${order.receiptNumber || order.id}`, 240, dnFootY + 12);
    doc.text(`Origin Facility: ${fulfillLoc} • Transport Seal Verified`, 240, dnFootY + 22);
    doc.text(`Generated by Taji Enterprise ERP • Document Version 2.4`, 240, dnFootY + 32);
    ...
    // Signatory footer with genuine QR & Barcode
    const sigY = bankBoxY + 95;
    if (qrDataUrl) {
      try {
        doc.addImage(qrDataUrl, 'PNG', 40, sigY, 44, 44);
      } catch (e) {
        console.warn('Failed to add QR image to invoice:', e);
      }
    }
    ...
    doc.setTextColor(100, 116, 139);
    doc.setFont('helvetica', 'normal');
    doc.text('KRA TIMS FISCAL VERIFICATION QR', 92, sigY + 10);
    doc.text(`Ref: ${order.receiptNumber} • KRA PIN: ${etrConfig.taxPin}`, 92, sigY + 19);
    doc.text(`CU Serial: ${etrConfig.cuSerialNumber || 'KRAMW019284'} • Fiscalized`, 92, sigY + 28);
    doc.text(`Generated by Taji Enterprise ERP • Document Version 2.4`, 92, sigY + 37);

    if (barcodeDataUrl) {
      try {
        doc.addImage(barcodeDataUrl, 'PNG', 265, sigY + 4, 130, 34);
      } catch (e) {
        console.warn('Failed to add barcode image to invoice:', e);
      }
    }
    ...
    doc.setFontSize(7.5);
    doc.setTextColor(15, 23, 42);
    doc.text('Authorized Signatory:', 425, sigY + 10);
    doc.setFont('helvetica', 'normal');
    doc.text('_____________________________', 425, sigY + 24);
    doc.text(`For ${etrConfig.companyName}`, 425, sigY + 34);
  }
