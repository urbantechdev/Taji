...
export const UserProfileModal: React.FC = () => {
  ...
  const authProvider = adminUser?.authProvider || (adminUser?.email?.includes('gmail') ? 'google' : 'email');
  ...
  const handleResendVerification = async () => {
    ...
    try {
      ...
      setStatusMessage({ type: 'error', text: err?.message || 'Failed to resend verification email.' });
    } finally {
  ...
  const handleCheckVerification = async () => {
    ...
    try {
      ...
      if (res.isVerified) {
        setStatusMessage({ type: 'success', text: 'Email verified successfully with Firebase Authentication!' });
      } else {
        setStatusMessage({ type: 'error', text: res.message });
      }
    } catch (err: any) {
      setStatusMessage({ type: 'error', text: err?.message || 'Failed to check verification status.' });
    } finally {
      setIsVerifying(false);
    }
  };
  ...
  const handleManualVerify = () => {
    verifyEmailManual();
    setStatusMessage({ type: 'success', text: 'Email marked as verified in preview environment!' });
  };
    ...
    <div className="fixed inset-0 bg-slate-950/75 backdrop-blur-md flex items-end sm:items-center justify-center p-0 sm:p-4 z-50 animate-fadeIn overflow-y-auto">
      <div className="w-full h-full sm:h-auto max-h-[100dvh] sm:max-h-[92vh] max-w-2xl bg-white rounded-none sm:rounded-3xl border-0 sm:border border-slate-200 shadow-2xl overflow-hidden animate-scaleUp flex flex-col">
        ...
        <div className="p-5 sm:p-6 overflow-y-auto space-y-6 flex-1 text-slate-800">
          ...
          {/* Firebase Authentication & Email Verification Status Card */}
          <div className="bg-white border-2 border-slate-200 rounded-2xl p-4 sm:p-5 space-y-3.5 shadow-xs">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2">
                <ShieldCheck className="w-4 h-4 text-rose-600" />
                <h3 className="text-sm font-extrabold text-slate-900">Firebase Identity &amp; Email Verification</h3>
              </div>
              <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider flex items-center gap-1 ${
                ...
                {isEmailVerified ? <CheckCircle className="w-3 h-3" /> : <AlertCircle className="w-3 h-3" />}
                <span>{isEmailVerified ? 'Email Verified' : 'Pending Verification'}</span>
              </span>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
              <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-1">
                <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Auth Provider</span>
                <div className="flex items-center gap-2 font-bold text-slate-800">
                  <span className="capitalize">{authProvider} Single Sign-On</span>
                </div>
              </div>
            ...
            {!isEmailVerified ? (
              <div className="p-3.5 bg-amber-50/80 border border-amber-200 rounded-xl space-y-2.5">
                <div className="flex items-start gap-2 text-xs text-amber-900">
                  ...
                  <p>
                    <strong>Email Verification Required:</strong> We sent a verification link to <strong>{currentUser.email}</strong>. Please confirm your email to ensure uninterrupted access to billing and audits.
                  </p>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-2 pt-1">
                  <button
                    ...
                    className="px-3 py-1.5 bg-amber-600 hover:bg-amber-700 text-white rounded-lg text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5 shadow-2xs active:scale-95 disabled:opacity-50"
                  >
                    <Mail className="w-3.5 h-3.5" />
                    <span>{isVerifying ? 'Sending...' : 'Resend Verification Email'}</span>
                  </button>
                  ...
                  <button
                    ...
                    disabled={isVerifying}
                    className="px-3 py-1.5 bg-white hover:bg-slate-50 text-slate-700 border border-slate-300 rounded-lg text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5 shadow-2xs active:scale-95 disabled:opacity-50"
                  >
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-300 rounded-lg text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5 shadow-2xs active:scale-95"
                    title="Bypass in development preview container"
                  >
                    <Check className="w-3.5 h-3.5 text-emerald-600" />
                    <span>Verify (Preview Mode)</span>
                  </button>
                </div>
              </div>
            ) : (
              <div className="p-3 bg-emerald-50/80 border border-emerald-200 rounded-xl flex items-center justify-between text-xs text-emerald-900">
                <div className="flex items-center gap-2">
                  <CheckCircle className="w-4 h-4 text-emerald-600 shrink-0" />
                  <span>
                    <strong>Firebase Verified Account:</strong> Your email address has been confirmed. Full enterprise role privileges active.
                  </span>
                </div>
                <button
                  type="button"
                  ...
                  className="px-2.5 py-1 bg-white hover:bg-emerald-100 text-emerald-800 border border-emerald-300 rounded-lg font-bold text-[11px] cursor-pointer transition-all shrink-0"
                >
