...
export const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab }) => {
  ...
  const allNavItems: { id: NavTab; label: string; icon: React.ReactNode; badge?: string }[] = [
    ...
    },
    {
      ...
      icon: <TrendingUp className="w-8 h-8 stroke-[2.2]" />,
      badge: 'Live'
    },
    {
      id: 'branches',
    ...
    },
    {
      ...
      icon: (currentUser.role === 'admin' || currentUser.role === 'hr_manager') ? <Users className="w-8 h-8 stroke-[2.2]" /> : <FileText className="w-8 h-8 stroke-[2.2]" />,
    },
    {
      id: 'operators',
    ...
    },
    {
      ...
      icon: <Settings className="w-8 h-8 stroke-[2.2]" />,
    },
    {
      id: 'guide',
      label: 'User Guide & Manual',
      icon: <BookOpen className="w-8 h-8 stroke-[2.2]" />,
      badge: 'Help'
    }
  ];
                ...
                <span
                  className={`absolute left-0 top-2 bottom-2 w-1.5 rounded-r-full transition-all duration-300 ${
                    isActive
                      ...
                      : 'bg-slate-400/0 group-hover/btn:bg-slate-400/80 group-hover/btn:opacity-100'
                  }`}
                />
                ...
                <div className="flex items-center gap-3 pl-1 min-w-0 flex-1">
                  <span
                    className={`shrink-0 transition-all duration-200 group-hover/btn:scale-110 group-hover/btn:rotate-2 ${
                      isActive ? 'text-white drop-shadow-xs' : 'text-slate-400 group-hover/btn:text-slate-200'
                    }`}
                  >
                    {item.icon}
                  </span>
