...
export const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab }) => {
  ...
  const accountantMenuItems: {
    ...
    },
    {
      ...
      section: 'Costing & Trade',
      icon: <Ship className="w-5 h-5 stroke-[2.2]" />,
      badge: 'KRA Suite'
    },
    {
      id: 'debtors_aging',
    ...
    },
    {
      ...
      section: 'Tax & Reconciliation',
      icon: <Building className="w-5 h-5 stroke-[2.2]" />
    }
  ];
                    ...
                    <div className="flex items-center gap-2.5 pl-1 min-w-0 flex-1">
                      <span
                        className={`shrink-0 transition-all duration-200 group-hover/btn:scale-110 ${
                          isSelected ? 'text-white drop-shadow-xs' : 'text-rose-400 group-hover/btn:text-rose-300'
                        }`}
                      >
                        {item.icon}
                      </span>
